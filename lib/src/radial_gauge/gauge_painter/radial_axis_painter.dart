...
class _AxisPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    if (_axis.backgroundImage != null &&
      ...
      if (!_axis.canScaleToFit) {
        _radius = math.min(_axis._axisSize.width, _axis._axisSize.height) / 2;
        _rect = Rect.fromLTRB(
        ...
        _radius = _axis._radius;
        _rect = Rect.fromLTRB(
            _axis._axisCenter.dx - _radius,
            _axis._axisCenter.dy - _radius,
            _axis._axisCenter.dx + _radius,
            _axis._axisCenter.dx + _radius);
      }
    ...
    // Disables the load time axis animation once the axis line and its elements got animated
  ...
  void _drawAxisPath(Canvas canvas, double _startRadian, double _endRadian) {
    ...
    canvas.save();
    canvas.translate(_axis._axisCenter.dx, _axis._axisCenter.dy);
  ...
  Paint _getPaint() {
    final Paint _paint = Paint()
      ..color = _axis.axisLineStyle.color ??
          _gauge._radialGaugeState._gaugeTheme.axisLineColor
        ...
        if (!(i == 0 && _axis._sweepAngle == 360)) {
          _tickPaint.color = _axis.useRangeColorForAxis
              ? _axis._getRangeColor(_tickOffset.value) ??
                  ...
                  _gauge._radialGaugeState._gaugeTheme.majorTickColor
              : _axis.majorTickStyle.color ??
                  _gauge._radialGaugeState._gaugeTheme.majorTickColor;
      ...
      for (int i = 0; i < _length; i++) {
        ...
        _tickPaint.color = _axis.useRangeColorForAxis
            ? _axis._getRangeColor(_tickOffset.value) ??
                ...
                _gauge._radialGaugeState._gaugeTheme.minorTickColor
            : _axis.minorTickStyle.color ??
                _gauge._radialGaugeState._gaugeTheme.minorTickColor;
  ...
  void _drawAxisLabels(Canvas canvas) {
    ...
    for (int i = 0; i < _length; i++) {
      if (!((i == 0 && !_axis.showFirstLabel) ||
        ...
        final Color _labelColor = label.labelStyle.color ??
            _gauge._radialGaugeState._gaugeTheme.axisLabelColor;
