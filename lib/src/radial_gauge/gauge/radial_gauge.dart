...
/// Create radial gauge widget to displays numerical values on a circular scale. It has a rich set of features
/// such as axes, ranges, pointers, and annotations that are fully customizable and extendable.
/// Use it to create speedometers, temperature monitors, dashboards, meter gauges, multiaxis clocks, watches, activity gauges, compasses, and more.
///
/// ```dart
...
class _SfRadialGaugeState extends State<SfRadialGauge>
  ...
  void _needsRepaintAxis(RadialAxis oldAxis, RadialAxis newAxis) {
    ...
    if (newAxis.startAngle != oldAxis.startAngle ||
        ...
        newAxis.canScaleToFit != oldAxis.canScaleToFit ||
  ...
  void _needsAnimatePointers(RadialAxis oldAxis, RadialAxis newAxis) {
    for (num i = 0; i < newAxis.pointers.length; i++) {
      if (oldAxis.pointers[i].value != newAxis.pointers[i].value) {
        setState(() {
          // Sets the previous animation end value as current animation start value of pointer
